...
export function ContactForm() {
  ...
  if (status === "sent") {
    return (
      <div className="w8-card-hi p-8 md:p-10 text-center">
        <span className="inline-flex items-center justify-center w-14 h-14 rounded-full" style={{ background: "var(--w8-ember)", color: "var(--w8-on-accent)" }}>
  ...
  const inputCls =
    "w-full rounded-xl bg-[var(--w8-bg-3)] border border-[var(--w8-line)] px-4 py-3.5 text-sm text-[var(--w8-text)] placeholder:text-[var(--w8-muted)] outline-none transition-colors focus:border-[var(--w8-ember)]";
  const labelCls =
    "block text-xs font-semibold uppercase tracking-[0.14em] w8-muted-hi mb-2";
  const selectCls = `${inputCls} appearance-none cursor-pointer [&>option]:bg-[var(--w8-bg-2)] [&>option]:text-[var(--w8-text)]`;
